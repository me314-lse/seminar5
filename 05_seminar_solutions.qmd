...
We will be working together on parts 1 through 5. You will work on part 6 on your own, and we will discuss the solutions at the end of the seminar.
...
data_selection <- data %>%
    mutate(
        D = ifelse(X > 0, rbinom(N, 1, 0.7), rbinom(N, 1, 0.3)), # D as a function of X
        Y = Y1 * D + Y0 * (1 - D)
    )
```
...
```{r random_assignment}
# randomize via Bernoulli trial (simple random assignment):
data_random <- data %>%
    mutate(
        D = rbinom(N, 1, 0.5), # D is randomized
        Y = Y1 * D + Y0 * (1 - D)
    )
```
...
# use tidy approach and add confidence intervals:
lm(Y ~ D, data_selection) |>
    broom::tidy(conf.int = TRUE, conf.level = 0.99)

# use tidy approach and get more goodness-of-fit statistics:
lm(Y ~ D, data_selection) |>
    broom::glance() |>
    t() |> # transpose, just to make it fit in the terminal
    round(3)
```
...
# Difference-in-means
mean(data_random$Y[data_random$D == 1]) - mean(data_random$Y[data_random$D == 0])
...
summary(lm_robust(Y ~ D, data_random))
summary(lm_robust(Y ~ D + X, data_random))
```
...
datasummary_balance(
    ~treat,
    data = select(lalonde, where(is.numeric), -re78), # select only numeric variables but not the outcome
    stars = T
)
```
...
# Regression
lm_robust(re78 ~ treat, data = lalonde)
```
...
```{r lalonde_dag}
lalonde_dag <- dagify(
    re78 ~ treat + educ + age + nodegree,
    coords = list(
        x = c(re78 = 4, treat = 1, educ = 2, age = 4, nodegree = 3),
        y = c(re78 = 1, treat = 1, educ = 3, age = 3, nodegree = 3)
    ),
    exposure = "treat",
    outcome = "re78"
)
...
```{r lalonde_nodegree}
lm_robust(re78 ~ treat + nodegree, data = lalonde) |> summary()
lm_robust(re78 ~ treat, data = lalonde) |> summary()
```
...
lm_robust(re78 ~ treat + nodegree + re75, data = lalonde) |> summary()
```
...
lm_robust(re78 ~ treat * age + nodegree, lalonde) |> summary()
```
